...
For Optimism, we support the following methods:
  ...
  * [EVM JSON-RPC methods](/rpc-service/chains/chains-api/optimism/#evm-json-rpc-methods) — available for both Mainnet and Testnet.
  * [Flashblocks methods](/rpc-service/chains/chains-api/optimism/#flashblocks-methods) — available for Sepolia Testnet only.
...
Flashblocks is a core Optimism feature that provides preconfirmation of the upcoming block's transaction ordering (~200 milliseconds) before the full block is sealed (~2 seconds).
...
The following RPC methods can return Flashblocks specific data:
  ...
  * [`eth_getBlockByNumber`](/rpc-service/chains/chains-api/optimism/#eth_getblockbynumber-1) — with `pending`, retrieves the latest Flashblock.
  * [`eth_getTransactionReceipt`](/rpc-service/chains/chains-api/optimism/#eth_gettransactionreceipt-1) — retrieves receipts for preconfirmed transactions.
  * [`eth_getBalance`](/rpc-service/chains/chains-api/optimism/#eth_getbalance-1) — with `pending`, retrieves the address balance in the latest Flashblock.
  * [`eth_getTransactionCount`](/rpc-service/chains/chains-api/optimism/#eth_gettransactioncount-1) — with `pending`, retrieves the address nonce in the latest Flashblock.
  * [`eth_getTransactionByHash`](/rpc-service/chains/chains-api/optimism/#eth_gettransactionbyhash-1) — retrieves preconfirmed transactions by hash.
