...
## EVM methods
  ...
  * [`eth_getAssetBalance`]() — retrieves the balance of first class Avalanche Native Tokens on the C-Chain (excluding AVAX, which must be fetched with eth_getBalance).
  * [`eth_baseFee`]() — retrieves the base fee for the next block.
  * [`eth_maxPriorityFeePerGas`]() — retrieves the priority fee needed to be included in a block.
  * [`eth_getChainConfig`]() — retrieves chain config.
