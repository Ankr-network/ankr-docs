...
> **Retrieves all contract allowances of a particular wallet address.**
...
* `params` (array): request body parameters:
  ...
  * `walletAddress` (required; string): the wallet address to check for smart contract allowances; supports the Ethereum Name Service (ENS).
  * `blockchain` (optional; string): a chain or a combination of chains to query:
    * Single chain: `eth`, `bsc`, `fantom`, `avalanche`, `polygon`, `polygon_zkevm`, `arbitrum`, `syscoin`, `optimism`, `rollux`, `eth_goerli`, `polygon_mumbai`, `avalanche_fuji`.
...
Returns detailed metadata for contract allowances of the wallet address specified.
