...
* `params` (array): request body parameters:
  ...
  * `blockchain` (optional; string): a chain or a combination of chains to query:
    * Single chain: `arbitrum`, `avalanche`, `base`, `bsc`, `eth`, `fantom`, `flare`, `gnosis`, `optimism`, `polygon`, `polygon_zkevm`, `rollux`, `syscoin`, `zksync_era`, `avalanche_fuji`, `polygon_mumbai`.
    * Chains combination: `[arbitrum, avalanche, base, bsc, eth, fantom, flare, gnosis, optimism, polygon, polygon_zkevm, rollux, syscoin, zksync_era, avalanche_fuji, polygon_mumbai]`.
