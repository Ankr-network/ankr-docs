...
To deploy a rollup using a Testnet network, follow these steps:
  ...
  3. On the **Essentials** tab, select the following rollup parameters, and then click **Next**:
      ...
      * **Network**: Select your **Testnet** option:
        ...
        * **Persistent** (on *Arbitrum Orbit* and *OP Stack* only): Use a permanent testing environment to experiment with your dapps before deploying them to Mainnet.
...
To deploy a rollup using a Testnet network, follow these steps:
  ...
  3. On the **Essentials** tab, select the following rollup parameters, and then click **Next**:
      ...
      * **Data Availability Layer**: [Fraxtal DA](https://docs.frax.com/fraxtal/network/data-availability)
  ...
  4. On the **Configuration** tab, specify the following parameters, and then click **Next**:
     ...
     <img src="/docs/scale/gas-token-fraxtal.png" class="responsive-pic" width="500" />
  ...
  7. Depending on the Testnet option selected, there are two deployment paths:
      * **Trial**: You must have `1 ankrETH` in your wallet as anti-abuse measure.<br/>
      In the **Prove ankrETH** dialog, perform the following actions to start your rollup deployment:
        ...
        * In MetaMask, select the Ethereum address to associate with your account.
        * Sign with MetaMask wallet.
      * **Persistent**: On the Stripe checkout page, provide your card details, and then click **Subscribe** to deploy the configured rollup.
