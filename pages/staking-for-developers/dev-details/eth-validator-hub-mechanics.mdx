...
## Architecture
Validator Hub is implemented as a set of smart contracts, backend, and frontend.
...
The attestation rewards are accumulated in portions, on average each day (24h).
...
2. Once the assets are available for claiming, the user claims them via `vStaker::claim(). Note that the assets become available at a different timing: unstake become available in 5 days on average, MEV & tips become available instantly, attestation rewards become available in 1 day on average.