...
Ankr provides an additional opportunity to operate the LST tokens by introducing a flash unstake feature.
This feature significantly decreases the unstake time — the user receives their unstaked BNBs in the same transaction.
...
## Smart contracts
* BNStakingPool — the main entry point to BNB Liquid Staking; stake, unstake, flash-unstake, etc.
...
#### Testnet
* [BNStakingPool](https://testnet.bscscan.com/address/0x0eCf14f54C5fF190e025Bc5e80c6351F91BFcB1c) (0x0eCf14f54C5fF190e025Bc5e80c6351F91BFcB1c)
...
// calculate the amount of BNBs with no flash unstake fee
...
// Calculate the final amount of BNBs the user gets after the fee subtraction
```

##### Parameters
`amount` (uint256, required) — amount of ankrBNBs to convert to BNBs.
...
#### `await BNBStakingPool.flashPoolCapacity()`
...
* `receiverAddress` (address; required) — the user's address to release the unstaked BNBs to.
