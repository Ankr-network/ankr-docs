...
## Rate per token
You can check the amount of original assets you get when you redeem an amount of a Liquid Staking Token.
To check it per token, use the `rateProvider` contracts:
...
For your convenience, there an explanation of `getRatioFor()` and contract address in a single place.

You can get ratio for bridged tokens via the `getRatioFor()` function. The common formula is `ankrETH/ratio=ETH`, etc.<br/>
