...
2. For withdrawals from validators (exit), we create long-lived tokens with a higher security level, only whitelisting our server's IPs in Cubists to accept such tokens and restricting the unstake amount. In case our users make a very big unstake, and we need to temporarily increase the amount, the majority of hardware token holders within the company have to sign to create a new token.
