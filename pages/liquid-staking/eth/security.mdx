...
Second, to send any signing requests it we use short-lived and long-lived keys that we issue via **multiple hardware tokens**.
...
The mechanics is very straight:
1. Our server calculates what needs to be signed.
2. We send a signing request to HSM that runs checks, including the double-signing check.
3. If all checks pass, HSM signs the data. Otherwise, we're immediately alerted if any malicious actions have been detected.

## Additional details
HSM is protected from physical damage by being replicated in multiple geo-locations. In case of an unlikely emergency event where all the HSMs are damaged, we can apply the majority of our hardware tokens to decrypt an HSM backup.

In case our users make a very big unstake, we can temporarily adjust the security policy by using our hardware keys.
...
For more information, read "[Securing staking keys properly](https://cubist.dev/blog/staking-keys-status-quo)" on Cubist's blog.
